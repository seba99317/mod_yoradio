...
//    Serial.printf("#CONTROL#: feels like: %+.0fC\n", tempfl);
  ...
  cursor = tmps;
      pressi = (float)atoi(press) - g_height;
...
tmps = strstr(cursor, "\"pressure\":");
bool pressure_pr = (tmps != NULL);
if (pressure_pr) {
    tmps += 11; // Przesuni?cie o d?ugo?? ci?gu "\"pressure\":"
    tmpe = strstr(tmps, "},");
    tmpc = strstr(tmps, ",\""); // lub adres do [},]

    if (tmpe == NULL) {
        Serial.println("##WEATHER###: pressure not found!");
    }

    // Kopiujemy warto?? ci?nienia z API (w hPa) do zmiennej `press`
    strlcpy(press, tmps, tmpe - tmps + (tmpc > tmpe ? 1 : (tmpc - tmpe + 1)));

    // `pressi` zawiera warto?? w hPa, tak? jak w API
