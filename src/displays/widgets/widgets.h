...
public:
  ...
  void loop();
private:
  int16_t _peakL;                    // Używam int16_t, aby móc przechowywać wartości ujemne tymczasowo przy opadaniu
  int16_t _peakR;                    // zanim zostaną zresetowane do 0.
  const uint16_t _peakFallRate = 2;  // Szybkość opadania wskaźnika szczytu (o ile pikseli na "tick")
  uint8_t _peakFallDelayCounter;     // Licznik do opóźniania opadania szczytu
  const uint8_t _peakFallDelay = 2;  // Co ile cykli ma opadać wskaźnik szczytu (np. co 3 cykle _draw())
