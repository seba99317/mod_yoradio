...
/**
 * Helper function to scan networks and select the strongest one with the given SSID.
 */
bool selectStrongestNetwork(const char *ssid, uint8_t *bssid) {
    int16_t bestRSSI = -1000;
...
/**
 * Modified to select the strongest network with the given SSID.
 */
wl_status_t WiFiSTAClass::begin(const char *ssid, const char *passphrase, int32_t channel, const uint8_t *bssid, bool connect) {
    ...
    // Jeśli BSSID nie jest podany, wyszukaj najsilniejszą sieć
    uint8_t selectedBSSID[6] = {0};
    if (bssid == nullptr) {
        if (!selectStrongestNetwork(ssid, selectedBSSID)) {
            return WL_CONNECT_FAILED;
        }
        bssid = selectedBSSID;
    }
